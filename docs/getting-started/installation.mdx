...
This runs `go install github.com/mithril-framework/mithril/cmd/mithril@main` and prints next steps.

Ensure `$(go env GOPATH)/bin` is on your `PATH` (before `/usr/local/bin` if an older `mithril` is installed).
...
`mithril new` clones the framework, removes dev cruft (stray binaries), rewrites the Go module to your project name, copies `env.example` → `.env`, runs `go mod tidy`, and initializes a **fresh git repository** (no link to framework history).
...
Ensure the framework CLI from `go install github.com/mithril-framework/mithril/cmd/mithril@main` is first on PATH. See [Quick Start troubleshooting](/docs/getting-started/quick-start#troubleshooting).
