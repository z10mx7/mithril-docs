...
This runs `GOPROXY=direct go install github.com/mithril-framework/mithril/cmd/mithril@v1.0.1` and verifies the installed binary. The install script bypasses stale `proxy.golang.org` cache (which may still serve v0.1.0 as `@latest`).

**After install**, add Go bin to your PATH (required if another `mithril` exists on the system):

```bash
export PATH="$(go env GOPATH)/bin:$PATH"
mithril --version   # expect: mithril 1.0.1 (github.com/mithril-framework/mithril)
```
...
```bash
go install github.com/mithril-framework/mithril/cmd/mithril@v1.0.1
mithril --version
```
...
`mithril new` clones the framework, removes dev cruft (stray binaries), rewrites the Go module (defaults to `github.com/<your-github-user>/<name>` when `gh` or `git config github.user` is set), copies `env.example` → `.env`, runs `go mod tidy`, and initializes a **fresh git repository**.
...
Ensure the framework CLI from `go install github.com/mithril-framework/mithril/cmd/mithril@v1.0.1` is first on PATH. See [Quick Start troubleshooting](/docs/getting-started/quick-start#troubleshooting).
