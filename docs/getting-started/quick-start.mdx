...
| http://localhost:4000/admin | Admin panel (after `mithril admin-enable`) |
| http://localhost:5050 | DBMS (after `mithril dbms-enable && mithril dbms`) |
...
```bash
# Login (after createsuperuser or seed)
