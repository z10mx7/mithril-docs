...
`mithril new` copies `env.example` to `.env` and sets `APP_NAME` to your project name. Use `-module` for a custom Go module path:
...
Or run `sudo $(go env GOPATH)/bin/mithril init` to symlink to `/usr/local/bin/mithril`.
