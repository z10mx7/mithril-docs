...
The `pkg/testing` helpers described below are planned. Use standard `go test`, `fiber.Test()`, and httptest as shown in the framework's `internal/auth/handlers_test.go` and `routes/core_test.go`.
</PlannedFeatureBanner>

Mithril uses Go's built-in testing package. Run tests with:
